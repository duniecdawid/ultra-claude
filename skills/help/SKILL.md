...
**Explain Like I Am An Idiot** (`/uc:explain-like-iam-idiot`)
Re-explains the current conversation in plain language, picking from a toolbox of explanation devices — bottom-line-first framing, analogies, ASCII flow/sequence/tree diagrams, comparison and weighted grids, named trade-offs — chosen to fit the material rather than following a fixed template. Use when you have lost the thread of a technical discussion, want something explained simply or ELI5, or need a recap before making a choice. Produces one plain-language message in the conversation where every option carries its cost alongside its benefit, so the decision is informed rather than merely summarized.
...
Orchestrates multi-task plan execution through per-task teams that self-coordinate from file-based context via the shared task-team-startup protocol — code tasks get Executor + Reviewer + Tester (executor model per task: sonnet/opus/fable), ops tasks a solo Executor — while Lead brokers mid-execution `ADVICE` and `QUERY` (external docs via /uc:research). Use after plan approval by running `/uc:plan-execution {number}`. Produces per-task `plan.md`, `impl.md`, and `test-strategy.md` artifacts, review and test verdicts, a Project Manager operational report, and a completion-time backlog review that auto-closes items the plan resolved (listing them) then triages each remaining detected issue per-question with a recommendation while the PM report is written — mid-execution user decisions ride a non-blocking escalation queue (`shared/escalations.md`) so an unattended run never stalls on a question.
...
Knowledge base for building harness components — skills, agents, hooks, protocols — plus a mandatory staged build workflow. Use when creating or refactoring a skill or agent, writing descriptions or prompts, auditing session context cost (`scripts/context_audit.py`), or optimising resident text. Build tasks edit in place on a clean git state and walk structural → lexical → description/name → compression (`uc:caveman-compress`) → present stages with per-stage discussion gates and a final `git diff` approval before commit; non-negotiables include the Opus floor, always invoking `/skill-creator:skill-creator`, description budgets, caveman house style from first draft, optimizing by removing instructions, and before/after refactor testing.
...
One machine-global background process that handles usage limits reactively — detects limit-killed turns via the StopFailure hook, tracks every account's reset time, and at each 5h or 7d reset wakes both what parked and any plan whose window merely ended in the soft band (a self-gated fleet parks nothing, so the Lead's pane injection is its only wake): durable `RESUME` signals into each task's `signals.jsonl` plus guarded tmux pane injection, with soft-band advisories and weekly-limit notifications. Installed and started by `/uc:setup`, self-healing via the SessionStart and StopFailure hooks, registered per plan. Execution rides each usage window to 100%, parks on the limit, and resumes automatically — no usage questions asked at plan start.
